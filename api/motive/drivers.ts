...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!enforceRateLimit(req, res, { windowMs: 60_000, maxRequests: 60, keyPrefix: 'motive-drivers' })) {
    return;
  }
  ...
  try {
    const response = await fetchWithRetry(
      `https://api.gomotive.com/v1/drivers?page_no=${page}&per_page=${per_page}`,
      {
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({
        error: 'Failed to fetch drivers from Motive',
    ...
    const normalized = normalizeIntegrationError(error, 'Failed to fetch drivers from Motive');
