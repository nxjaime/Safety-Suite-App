...
- Driver-role-specific portal verification remains incomplete without driver-role credentials; the non-driver portal leak has focused regression coverage and awaits production verification after the follow-up deployment, but the driver-owned-data happy path still needs a live driver session.
