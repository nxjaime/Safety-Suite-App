...
- Deploy and recheck the stale-session recovery fix before continuing live browser lifecycle QA.
...
| S67-001 | 67 | P1 | Auth/session recovery | Platform admin | Existing production browser session resumes Sprint 67 after stored Supabase token expiry | App accepted an expired stored session fallback, rendered authenticated shell/readonly Training, then `/admin` and `/drivers` stalled on `Loading...` while auth refresh repeatedly logged `TypeError: Failed to fetch` | Expired stored sessions should be cleared and treated as signed out so the user returns to `/login` instead of entering stale authenticated UI or hanging route loads | 67 | Local fix prepared in `AuthContext` with `src/test/authStoredSession.test.ts`; full local verification passed. Needs commit, deploy, and production recheck |
