...
- Local verification passed after the Safety fix: `npm test -- --run src/test/riskService.test.ts src/test/driverService.test.ts`, `npm run test:unit` (264 tests), `npm run lint` (139 existing warnings, 0 errors), `npm run build`, `npm run test:layout` (16 Playwright checks), and `npm run audit:prod` exited 0 at the high threshold. Audit now reports two moderate React Router advisories requiring a breaking React Router 7 upgrade.
...
| S67-002 | 67 | P1 | Safety risk-event logging | Platform admin | User submits `/safety` Log Risk Event with selected driver, event type, date, points, and notes | Production valid-submit path returned `Failed to log event`; Supabase reproduction showed `risk_events.points` NOT NULL violation because normalized ingestion omitted legacy `points` | Valid safety events should insert, update driver risk score/history, refresh Safety metrics/queue, and persist after reload | 67 | Local fix prepared to write legacy `points` and forward UI points into risk ingestion; focused tests, unit, lint, build, layout, and high-threshold audit passed. Needs commit, deploy, and production @browser recheck |
